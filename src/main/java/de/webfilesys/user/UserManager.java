...
public interface UserManager {
	 ...
	 * 
	 * @param receiverRole
	 *            the role of the users ("admin", "user" or "webspace")
	 * @return a vector of Strings (e-mail addresses)
	 */
	 ...
	 * 
	 * @param userId
	 *            the userId (=login name) of the new user
	 ...
	 * 
	 * @param userId
	 *            the userid (=login name) of the user to be deleted
	...
	/**
	 * When a user publishes a complete folder tree, a virtual user is created that has read-only access and
	 * can be used for login by guests. The path of the base folder is used as document root for the virtual
	 * user. The virtual user exists only for a limited time. The generated userid of the virtual user must be
	 * unique. It is the responsibility of the InvitationManager to remove expired virtual users.
	 * 
	 * @param realUser
	...
	/**
	 * The user type is used to flag virtual users that are created for published folder trees.
	 * 
	 * @param userId
	 *            the userid of the user
	 * @param type
	 *            the type of the user (for example "virtual")
	...
	/**
	 * Get the disk quota (bytes) of the user. If no disk quota is defined for the user, this method returns
	 * -1.
	 * 
	 * @param userId
	 *            the userid of the user
	 ...
	 *            the userid of the user
	 * @param newValue
	 *            the new disk quota value (bytes)
	 */
	public void setDiskQuota(String userId, long newValue);

	/**
	 * Get the number of pictures displayed per page. If not defined the method should return
	 * fmweb.thumbnailsPerPage .
	 * 
	 * @param userId
	 *            the userid of the user
	...
	/**
	 * Get the document root for the user. If the document root is not defined, this method must return a
	 * platform-dependend default value. For Windows this should be "c:\temp" and for UNIX "/tmp".
	 * 
	 * @param userId
	 *            the userid of the user
	...
	/**
	 * Creates a normalized form of the document root. Implemented in abstract base class UserManagerBase. Do
	 * not overwrite it!
	 * 
	 * @param documentRoot
	 ...
	 *            the userid of the user
	 * @param newCSS
	 *            the name of the new CSS stylesheet
	 */
	...
	/**
	 * The user manager thread is being notified before fmweb shutdown. If the user manager is ready for
	 * shutdown, it must return true here.
	 * 
