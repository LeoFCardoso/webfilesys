...
public class GetFileRequestHandler extends UserRequestHandler {
	public GetFileRequestHandler(HttpServletRequest req, HttpServletResponse resp, HttpSession session,
	...
	protected void process() {
		...
		Logger.getLogger(getClass()).info("User " + getUid() + " requested this file: " + filePath);
		...
		if (disposition == null) {
			ViewHandlerConfig viewHandlerConfig = ViewHandlerManager.getInstance().getViewHandlerConfig(
					fileToSend.getName());
		...
		if ((cached != null) && (cached.equals("true"))) {
			resp.setDateHeader("expires", System.currentTimeMillis() + (10 * 60 * 60 * 1000)); // now + 10
																								// hours
		}
		...
		try {
			...
			if (WebFileSys.getInstance().isDownloadStatistics()
					&& (filePath.indexOf(ThumbnailThread.THUMBNAIL_SUBDIR) < 0)) {
