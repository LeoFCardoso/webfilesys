...
public class XslRequestHandlerBase extends UserRequestHandler {
	...
	public XslRequestHandlerBase(HttpServletRequest req, HttpServletResponse resp, HttpSession session,
	 ...
	 */
	protected void addAlbumPath(String actPath, Element rootElement, Element currentPathElem) {
	...
	/**
	 * This problem seems to be history with a current version of firefox: Mozilla browsers (Firefox,
	 * Netscape) have a very strange bug in the XSLT processor. HTML select boxes generated from XSL do not
	 * work in most cases, the selected value cannot be changed and the onchange() event handler is not
	 * called.
	 * 
	 * If the resulting HTML page contains a HTML select form element and the client is Mozilla, we have to do
	 * the XSL processing on the server.
	 * 
	 * @param xslFile
	...
	public void processResponse(String xslFile, boolean handleMozillaXslBug) {
		...
		// && ((!handleMozillaXslBug) || (browserManufacturer != BROWSER_MOZILLA)))
		{
			...
			try {
				Transformer transformer = tf.newTransformer(new StreamSource(new File(xslPath)));

				transformer.setOutputProperty(OutputKeys.ENCODING, "UTF-8");

				// Leonardo - context path name to be used on XSL's
