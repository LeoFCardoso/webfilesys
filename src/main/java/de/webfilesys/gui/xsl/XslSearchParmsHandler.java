...
public class XslSearchParmsHandler extends XslRequestHandlerBase {
	public XslSearchParmsHandler(HttpServletRequest req, HttpServletResponse resp, HttpSession session,
	...
	protected void process() {
		...
		if (!accessAllowed(currentPath)) {
			Logger.getLogger(getClass()).warn(
					"user " + uid + " tried to access folder outside of his document root: " + currentPath);
		...
		ProcessingInstruction xslRef = doc.createProcessingInstruction("xml-stylesheet",
				"type=\"text/xsl\" href=\"" + req.getContextPath() + "/xsl/searchParms.xsl\"");
		...
		addMsgResource("label.argdesc1",
				getResource("label.argdesc1", "searches for files that contain all of the words above"));
		addMsgResource("label.argdesc2", getResource("label.argdesc2", "search phrase containing spaces"));
		addMsgResource("label.includeSubdirs",
				getResource("label.includeSubdirs", "include subdirectories in search"));
		addMsgResource("label.includemetainf",
				getResource("label.includemetainf", "include meta info/description in search"));
		addMsgResource("label.metainfonly",
				getResource("label.metainfonly", "search in meta info/description only"));
		...
		addMsgResource("label.searchCalendar",
				getResource("label.searchCalendar", "select date from calendar"));
		addMsgResource("label.searchDateConflict",
				getResource("label.searchDateConflict", "the end date must be after start date!"));

		addMsgResource("label.assignedToCategory",
				getResource("label.assignedToCategory", "assigned to category"));
		addMsgResource("label.selectCategory", getResource("label.selectCategory", "- select categories -"));

		addMsgResource("label.searchResultAsTree",
				getResource("label.searchResultAsTree", "show results as folder tree"));
