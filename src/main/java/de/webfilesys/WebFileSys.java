...
public class WebFileSys {
	...
	public static final String VERSION = "Version 2.9.5-L (2016/07)";
	...
	/**
	 * allow chmod/chown for users of role webspace on special request from chat3you
	 */
	private boolean chmodAllowed = false;

	/**
	 * Are backward links from the linked file to the linking file enabeld? Linking backward from the link
	 * target file to the link is required to allow automatic update of the link when the target file is moved
	 * so that the link points to the new target path after the move operation. Reverse linking has some
	 * disadvantages too: A metainf file is created in the folder containing the link taget file (if it not
	 * already exists). And there is some additional processing required for move operations (performance!).
	 */
	private boolean reverseFileLinkingEnabled = false;

	private DiskQuotaInspector quotaInspector = null;

	private String pdfViewHandlerWatermark = "--==789==--";
	...
	private WebFileSys(Properties config, String webAppRootDir) {
		...
		if (openRegistration) {
			if (userDocRoot != null) {
				...
				if ((!docRootFile.exists()) || (!docRootFile.isDirectory()) || (!docRootFile.canWrite())) {
					Logger.getLogger(getClass()).error(
							"UserDocumentRoot is not a writable directory: " + userDocRoot);
					userDocRoot = null;
				} else {
					if ((File.separatorChar == '\\') && (userDocRoot.length() > 2)) {
						try {
							...
							if (!canonicalRoot.equals(absoluteRoot)) {
								Logger.getLogger(getClass()).error(
										"UserDocumentRoot is not a writable directory (check uppercase/lowercase!): "
												+ userDocRoot);
								userDocRoot = null;
							}
			...
			if (userDocRoot == null) {
				...
				Logger.getLogger(getClass()).info(
						"using default UserDocumentRoot for open registration: " + userDocRoot);
			}
		}
		...
		if (openRegistration) {
			...
			try {
				...
				Logger.getLogger(getClass()).error(
						"invalid default disk quota value: " + temp + " - using default value "
								+ DEFAULT_DISK_QUOTA);
			}
		}
		...
		if (temp.equalsIgnoreCase("true")) {
			...
			if (!CommonUtils.isEmpty(temp)) {
				try {
					...
					Logger.getLogger(getClass()).error(
							"invalid property value for MaxAppointmentMailsPerHour: " + temp);
					maxAppointmentMailsPerHour = DEFAULT_MAX_APP_MAILS_PER_HOUR;
				}
			}
			Logger.getLogger(getClass()).debug(
					"maximum allowed appointment e-mails per hour: " + maxAppointmentMailsPerHour);
			...
			if (!CommonUtils.isEmpty(temp)) {
				try {
					...
					Logger.getLogger(getClass()).error(
							"invalid property value for AppointmentExpirationDays: " + temp);
		...
		if (File.separatorChar == '/') {
			...
			if (jpegtranPath != null) {
				...
				if ((!jpegTranExe.exists()) || (!jpegTranExe.isFile()) || (!jpegTranExe.canRead())) {
					Logger.getLogger(getClass()).error("jpegtran executable " + jpegtranPath + " not found");
		...
		// PDF Watermark - Leonardo
		temp = config.getProperty("PDFViewHandlerWatermark");
		if (temp != null) {
			pdfViewHandlerWatermark = temp;
			Logger.getLogger(getClass()).info("PDF Watermark: " + pdfViewHandlerWatermark);
		}

		// JAAS Configurations
		temp = config.getProperty("JAASAppName");
		if (temp != null) {
			jaasAppName = temp;
	...
	public void initialize(Properties config) {
		if ((userMgrClass == null) || (userMgrClass.trim().length() == 0)) {
			...
			try {
				...
				// Fix Leonardo - error -> info
				Logger.getLogger(getClass()).info("User Manager class: " + this.userMgrClass);
			} catch (ClassNotFoundException cnfex) {
				Logger.getLogger(getClass()).error(
						"the user manager class " + userMgrClass + " cannot be found: " + cnfex);
				...
				Logger.getLogger(getClass())
						.error("the class " + userMgrClass
								+ " does not implement the UserManager interface: " + cex);
			}
		}
	...
	public boolean is32bitWindows() {
		return ((opSysName != null))
				&& (opSysName.equals("Windows NT") || opSysName.equals("Windows XP")
						|| opSysName.equals("Windows 2003") || opSysName.equals("Windows Vista")
						|| opSysName.equals("Windows 7") || opSysName.equals("Windows 2000"));
	}
	...
	public String getPdfViewHandlerWatermark() {
		return pdfViewHandlerWatermark;
	}

	public String getJAASAppName() {
		return jaasAppName;
	}
