...
public class GeoTrackViewHandler implements ViewHandler {
			...
			while (!documentEnd) {
				try {
					...
					case XMLStreamConstants.START_DOCUMENT:
						/*
						 * xmlOut.println(XML_HEADER); xmlOut.println(STYLESHEET_REF);
						 */
						break;
					...
					case XMLStreamConstants.START_ELEMENT:
						...
						if ((!tagName.equals("gpx")) && (!tagName.equals("trk"))
								&& (!tagName.equals("trkseg")) && (!tagName.equals("trkpt"))
								&& (!tagName.equals("wpt")) && (!tagName.equals("ele"))
						...
						final String STYLESHEET_REF = "<?xml-stylesheet type=\"text/xsl\" href=\""
								+ req.getContextPath() + "/xsl/gpxProfile.xsl\"?>";
						...
						if (tagName.equals("trkpt") || tagName.equals("wpt")) {
							...
							if ((lat != null) && (lon != null)) {
								...
								try {
									...
									if ((startPointLat == Double.MIN_VALUE)
											|| (startPointLon == Double.MIN_VALUE)) {
										distFromStart = 0.0f;
										startPointLat = latitude;
										startPointLon = longitude;
									} else {
										distFromStart = calculateDistance(startPointLat, startPointLon,
												latitude, longitude);
									}
	...
	double calculateDistance(double ax, double ay, double bx, double by) {
		...
		return (1000.0f * d * (1.0f + x * h1 * Math.pow(Math.sin(f), 2) * Math.pow(Math.cos(g), 2) - x * h2
	...
	public void processZipContent(String zipFilePath, InputStream zipIn, ViewHandlerConfig viewHandlerConfig,
		...
		Logger.getLogger(getClass()).warn(
				"reading from ZIP archive not supported by ViewHandler " + this.getClass().getName());
	}
