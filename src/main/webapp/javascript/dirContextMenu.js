...
function dirContextMenu(domId) {
	...
	// Disable stats menu option - Leonardo
	// menuText = menuText
	// + menuEntry("javascript:statisticsMenu('"
	// + insertDoubleBackslash(shortPathName) + "', '"
	// + scriptPreparedPath + "')",
	// resourceBundle["label.statistics"] + ' >', null);

	// Disable search menu option - Leonardo
	// menuText = menuText
	// + menuEntry("javascript:search('" + scriptPreparedPath +
	// "')",resourceBundle["label.search"],null);
...
function extendedDirMenu(shortPath, path) {
	...
	if (((parent.serverOS == 'win') && (path.length > 3))
			|| ((parent.serverOS == 'ix') && (path.length > 1))) {
		menuText = menuText
				+ menuEntry("/webfilesys/servlet?command=downloadFolder&path="
