...
function jsLinkMenu(linkName, realPath) {
	...
	if (fileExt == ".ZIP" || fileExt == ".JAR" || fileExt == ".WAR") {
		...
		if (fileExt == ".URL") {
			menuText = menuText
					+ menuEntry("<%=request.getContextPath()%>/servlet?command=openUrlFile&actPath="
							+ encodeURIComponent(realPath) + "&random=" + (new Date().getTime()),
	...
	if (parent.clientIsLocal != 'true') {
		...
		menuText = menuText
				+ menuEntry("<%=request.getContextPath()%>/servlet?command=getFile&filePath="
						+ encodeURIComponent(realPath) + "&disposition=download", downloadLabel, null);
	}
	...
	if (parent.readonly != 'true') {
		...
		if (parent.clientIsLocal == 'true') {
			...
			menuText = menuText
					+ menuEntry("javascript:editRemoteLink('" + scriptPreparedPath + "')",
		...
		if (parent.serverOS == 'win') {
			if (parent.webspaceUser != 'true') {
				if ((fileExt == ".EXE") || (fileExt == ".COM") || (fileExt == ".BAT") || (fileExt == ".CMD")) {
					menuText = menuText
							+ menuEntry("<%=request.getContextPath()%>/servlet?command=execProgram&progname="
									+ encodeURIComponent(realPath), resourceBundle["label.run"], null);
				} else {
					menuText = menuText
							+ menuEntry("javascript:associatedProg('" + scriptPreparedPath + "')",
			...
			if (parent.webspaceUser != 'true') {
				menuText = menuText
						+ menuEntry("javascript:accessRights('" + scriptPreparedPath + "')",
		...
		if (parent.serverOS == 'win') {
			menuText = menuText
					+ menuEntry("javascript:switchReadWrite('" + scriptPreparedPath + "')",
	...
	menuText = menuText
			+ menuEntry("javascript:origDir('" + insertDoubleBackslash(realDir) + "')",
					resourceBundle["label.origDir"], null);
...
function editRemoteLink(path) {
	...
	editWin = window.open("<%=request.getContextPath()%>/servlet?command=editFile&filePath=" + encodeURIComponent(path)
			+ "&screenHeight=" + editWinHeight, "editWin", "status=no,toolbar=no,location=no,menu=no,width="
			+ editWinWidth + ",height=" + editWinHeight + ",resizable=yes,left=20,top=5,screenX=20,screenY=5");
	editWin.focus();
	editWin.opener = self;
}

function origDir(path) {
	parent.parent.frames[1].location.href = "<%=request.getContextPath()%>/servlet?command=exp&expandPath="
			+ encodeURIComponent(path) + "&fastPath=true";
}

function emailLink(filePath) {
	showPrompt(
			'<%=request.getContextPath()%>/servlet?command=emailFilePrompt&filePath=' + encodeURIComponent(filePath),
			'<%=request.getContextPath()%>/xsl/emailFile.xsl', 400, 250);
