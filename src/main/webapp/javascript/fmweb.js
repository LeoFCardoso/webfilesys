...
function multiFileCopyMove(cmd) {
	...
	xmlRequestPost("<%=request.getContextPath()%>/servlet",
			getFormData(document.form1), showCopyResult);
		...
		compareWin = window
				.open(
						...
						'width='
								...
								+ ',height='
								+ (screen.height - 80)
								+ ',scrollbars=yes,resizable=yes,status=no,menubar=no,toolbar=no,location=no,directories=no,screenX=0,screenY=0,left=0,top=0');
		compareWin.focus();
...
function validateBookmarkName(errorMsg) {
	...
	if (bookmarkName != '') {
		var createBookmarkUrl = '<%=request.getContextPath()%>/servlet?command=createBookmark&path='
				+ encodeURIComponent(document.bookmarkForm.currentPath.value)
				+ '&bookmarkName='
				+ encodeURIComponent(document.bookmarkForm.bookmarkName.value);
...
function switchFolderWatch(path) {
	var url = "<%=request.getContextPath()%>/servlet?command=switchFolderWatch&path="
...
function bookmark(path) {
	if (path && (path.length > 0)) {
		showPrompt(
				'<%=request.getContextPath()%>/servlet?command=addBookmark&path='
						+ encodeURIComponent(path),
				'<%=request.getContextPath()%>/xsl/addBookmark.xsl', 320, 190);
	} else {
		showPrompt('<%=request.getContextPath()%>/servlet?command=addBookmark',
				'<%=request.getContextPath()%>/xsl/addBookmark.xsl', 320, 190);
	}
...
function renameLink(linkName) {
	var promptDialog = showPromptDialog(
			"<%=request.getContextPath()%>/html/renameLink.jsp", 360);

	document.getElementById("oldLinkName").value = linkName;
	document.getElementById("oldLinkNameShort").innerHTML = shortText(linkName,
			35);
