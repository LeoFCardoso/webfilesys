...
function contextMenu(fileName) {
	...
	if (fileName.length > 24) {
		shortFileName = fileName.substring(0, 7) + "..."
	...
	if (fileExt == ".ZIP" || fileExt == ".JAR" || fileExt == ".WAR"
			|| fileExt == ".EAR") {
		menuText = menuText
				+ menuEntry("javascript:viewZip('" + scriptPreparedPath + "')",
						resourceBundle["label.viewzip"], null);
	} else {
		if (fileExt == ".URL") {
			menuText = menuText
					+ menuEntry(
	...
	// Disable download menu option - Leonardo
	...
	if (parent.readonly != 'true') {
		menuText = menuText
				+ menuEntry("javascript:delFile('" + scriptPreparedFile + "')",
						resourceBundle["label.delete"], null);

		menuText = menuText
				+ menuEntry("javascript:renameFile('" + scriptPreparedFile
						+ "')", resourceBundle["label.renameFile"], null);

		menuText = menuText
				+ menuEntry("javascript:copyToClipboard('" + scriptPreparedFile
						+ "')", resourceBundle["label.copyToClip"], null);

		menuText = menuText
				+ menuEntry("javascript:cutToClipboard('" + scriptPreparedFile
						+ "')", resourceBundle["label.cutToClip"], null);

		if (parent.clientIsLocal == 'true') {
			menuText = menuText
					+ menuEntry("javascript:editLocal('" + scriptPreparedFile
							+ "')", resourceBundle["label.edit"], null);
		} else {
			menuText = menuText
					+ menuEntry("javascript:editRemote('" + scriptPreparedFile
							+ "')", resourceBundle["label.edit"], null);
		}
		...
		if ((fileExt == ".ZIP") || (fileExt == ".JAR") || (fileExt == ".WAR")
			...
			menuText = menuText
					+ menuEntry("javascript:zip('" + scriptPreparedPath + "')",
							resourceBundle["label.unzip"], null);
		} else {
			if ((fileExt == ".GZ") || (fileExt == ".GZIP")) {
				menuText = menuText
						+ menuEntry("javascript:gunzip('" + scriptPreparedPath
								+ "')", resourceBundle["label.unzip"], null);
			} else {
				menuText = menuText
						+ menuEntry("javascript:zip('" + scriptPreparedPath
								+ "')", resourceBundle["label.zip"], null);
			}
		}

		if (fileExt == ".TAR") {
			menuText = menuText
					+ menuEntry("javascript:untar('" + scriptPreparedPath
							+ "')", resourceBundle["label.untar"], null);
		}
		...
		if (parent.serverOS == 'ix') {
			if (parent.webspaceUser != 'true') {
				if (fileExt == ".Z") {
					menuText = menuText
							+ menuEntry("javascript:compress('"
									+ scriptPreparedPath + "')",
									resourceBundle["label.uncompress"], null);
				} else {
					menuText = menuText
							+ menuEntry("javascript:compress('"
									+ scriptPreparedPath + "')",
									resourceBundle["label.compress"], null);
				}
			}
		} else // win
		{
			menuText = menuText
					+ menuEntry("javascript:switchReadWrite('"
							+ scriptPreparedPath + "')",
							resourceBundle["label.switchReadOnly"], null);
		}

		if (parent.mailEnabled == 'true') {
			menuText = menuText
					+ menuEntry("javascript:sendFile('" + scriptPreparedFile
							+ "')", resourceBundle["label.sendfile"], null);
		}

		if (fileExt == ".MP3") {
			menuText = menuText
					+ menuEntry("javascript:editMP3('" + scriptPreparedPath
							+ "')", resourceBundle["label.editmp3"], null);
		} else {
			menuText = menuText
					+ menuEntry("javascript:description('" + scriptPreparedPath
							+ "')", resourceBundle["label.editMetaInfo"], null);
		}

	}

	// Disable comments menu option - Leonardo
	// menuText = menuText
	// + menuEntry("javascript:comments('" + scriptPreparedPath + "')",
	// resourceBundle["label.comments"], null);

	// Disable diff menu option - Leonardo
	// if (parent.readonly == 'true') {
	// menuText = menuText
	// + menuEntry("javascript:diffSelect('" + scriptPreparedPath
	// + "')", resourceBundle["label.diffSource"], null);
	// } else {
	// menuText = menuText
	// + menuEntry("javascript:extendedFileMenu('"
...
function extendedFileMenu(shortFileName, path) {
	...
	if (parent.serverOS == 'win') {
		if (parent.adminUser == 'true') {
			if ((fileExt == ".EXE") || (fileExt == ".COM")
				...
				menuText = menuText
						+ menuEntry("javascript:associatedProg('"
								+ scriptPreparedPath + "')",
								resourceBundle["label.open"], null);
			}
		}
	} else {
		if ((parent.webspaceUser != 'true') || (parent.chmodAllowed == 'true')) {
			menuText = menuText
					+ menuEntry("javascript:accessRights('"
							+ scriptPreparedPath + "')",
							resourceBundle["label.rights"], null);
		}
	...
	if (parent.readonly != 'true') {
		menuText = menuText
				+ menuEntry("javascript:cloneFile('" + scriptPreparedFile
						+ "')", resourceBundle["label.cloneFile"], null);
	}

	menuText = menuText
			+ menuEntry("javascript:hexView('" + scriptPreparedFile + "')",
					resourceBundle["label.hexView"], null);

	if (fileExt == ".AES") {
		menuText = menuText
				+ menuEntry("javascript:decrypt('" + scriptPreparedFile + "')",
						resourceBundle["label.decrypt"], null);
	} else {
		menuText = menuText
				+ menuEntry("javascript:encrypt('" + scriptPreparedFile + "')",
						resourceBundle["label.encrypt"], null);
	}

	menuText = menuText
			+ menuEntry("javascript:tail('" + scriptPreparedPath + "')",
					resourceBundle["label.tail"], null);

	if (parent.readonly != 'true') {
		menuText = menuText
				+ menuEntry("javascript:touch('" + scriptPreparedFile + "')",
						resourceBundle["label.touch"], null);
	}

	menuText = menuText
			+ menuEntry("javascript:grep('" + scriptPreparedPath + "', '"
					+ scriptPreparedFile + "')", resourceBundle["label.grep"],
					null);
...
function diffSelectTarget(path, shortFileName, scriptPreparedPath) {
	...
	menuText = menuText
			+ menuEntry("javascript:diffSelect('" + scriptPreparedPath + "')",
					resourceBundle["label.diffTarget"], null);

	menuText = menuText
			+ menuEntry("javascript:cancelDiff('" + scriptPreparedPath + "')",
					resourceBundle["label.cancelDiff"], null);
...
function positionMenuDiv(menuDiv, maxMenuHeight) {
	if (browserFirefox) {
		clickYPos = clickYPos
				- document.getElementById('fileListTable').scrollTop + 100;
		clickXPos = clickXPos
				- document.getElementById('fileListTable').scrollLeft;
	}
